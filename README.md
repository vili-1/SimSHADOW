...
- Direct measurement in each observable's eigenbasis (no full classical  reconstruction)
...
```bash
python -m venv P_env
source P_env/bin/activate  # On Windows: P_env\Scripts\activate
...
- Save timestamped results to `results/P_results_*.json` and detailed logs to `logs/`
...
***5.1 EQ1 run_P.py over many repeats***
...
For example:
```
nohup ./Evaluation_NIER/scripts_RQ1/generate_n_fingerprints.sh /users/kevenmen/P_artifact-main 1000 > all_16012026.log 2>&1 &
```
...
```bash
ls -1 results/P_results_*.json | head -n 3
ls -1 results/P_report_*.txt | head -n 3
ls -1 logs/P_experiment_*.log | head -n 3
...
- `results/P_results_*.json` (experiment outputs)
- `results/P_report_*.txt` (human-readable summaries)
- `logs/P_experiment_*.log` (execution logs)
- `figures/figure2_fingerprints_IBM.pdf`
- `figures/figure2_fingerprints_IBM_QuantinuumH2.pdf`
- `figures/figure3_scaling.pdf`
- `figures/figure3_scaling.pdf`
...
- `results/P_results_*.json` (raw experiment outputs)
- `results/P_report_*.txt` (human-readable summaries)
- `logs/P_experiment_*.log` (execution logs)
